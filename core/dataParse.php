...
$aktuellesSkript = basename($_SERVER['SCRIPT_NAME'], ".php");
...
if (isset($_SESSION['application'])) {
    ...
    // Schauen ob es für das Skript Daten gibt die noch nicht in der Session sind
    if (!isset($_SESSION['page_' . $aktuellesSkript]) && file_exists($path . "seiten/" . $aktuellesSkript . ".yml")) {

        $yaml .= file_get_contents($path . "seiten/" . $aktuellesSkript . ".yml");
        $yaml .= "\n";
    }

    $parsed = array_merge($_SESSION, parseYaml($yaml));
    $_SESSION = $parsed;
    $_SESSION['page_' . $aktuellesSkript] = true;
} else {
    ...
    while (($file = $dir->read()) !== false) {
        ...
        if (strlen($file) <  5 or $file == ".DS_Store" or $file == "seiten")
            continue;
        // Make sure to skip linked data if set so
        if (isset($skipLinkedData) && $file == "linkedData.yml") {
            continue;
        }
    ...
    // Schauen ob es für das Skript Daten gibt die noch nicht in der Session sind
    if (file_exists($path . "seiten/" . $aktuellesSkript . ".yml")) {

        $yaml .= file_get_contents($path . "seiten/" . $aktuellesSkript . ".yml");
        $yaml .= "\n";
    }

    // Schauen ob daten aus anderen Seiten geladen werden sollen 
    if (isset($seitendaten)) {
        foreach ($seitendaten as $key => $seite) {
            if (file_exists($path . "seiten/" . $seite . ".yml")) {

                $yaml .= file_get_contents($path . "seiten/" . $seite . ".yml");
                $yaml .= "\n";
            }
        }
    }
...
function getPageYaml() {
}

function arrayToYaml($array) {
    $parsed = Spyc::YAMLDump($array);
    return $parsed;
}
...
function get_spreadsheetData($url, $var, $sheet = false) {
    ...
    try {
        ...
        if (!$handle) {
            throw new Exception('Failed to open spreadsheet ' . $url . '  <b>Are you offline?</b>');
        }
        // send success JSON
